...
export default async function ProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [relation, friendship, friendIds] = await Promise.all([
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-black/60 dark:text-white/60">
              ...
              {profileUser.gender && ` · ${GENDER_LABELS[profileUser.gender as Gender] ?? profileUser.gender}`}
              {profileUser.age !== null && ` · ${profileUser.age} lat`}
            </p>
          </div>
        </div>
